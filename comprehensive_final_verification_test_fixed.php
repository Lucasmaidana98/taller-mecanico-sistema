...
/**
 * COMPREHENSIVE FINAL VERIFICATION TEST - FIXED VERSION
...
function makeCurlRequest($url, $method = 'GET', $data = null, $cookieFile = null, $headers = [], $followRedirects = false) {
    $ch = curl_init();
...
function loginUser($email, $password, $cookieFile) {
    ...
    // Attempt login (don't follow redirects to detect 302)
    ...
    // Success is indicated by 302 redirect
...
$adminCookieFile = '/tmp/admin_crud_cookies_fixed.txt';
...
if (loginUser($testUsers['admin']['email'], $testUsers['admin']['password'], $adminCookieFile)) {
    ...
    $testResults['crud_operations']['admin_login'] = 'PASS';
    
    // Test Clientes UPDATE operations
    echo "\n--- Testing Clientes UPDATE operations ---\n";
    
    // Get clientes list to find a record to update
    $clientesResponse = makeCurlRequest("$baseUrl/clientes", 'GET', null, $adminCookieFile, [], true);
    if ($clientesResponse['http_code'] === 200) {
        ...
        if (preg_match('/\/clientes\/(\d+)\/edit/', $clientesResponse['response'], $matches)) {
            ...
            // Get edit form
            $editResponse = makeCurlRequest("$baseUrl/clientes/$clienteId/edit", 'GET', null, $adminCookieFile, [], true);
    ...
    $clientesCreateResponse = makeCurlRequest("$baseUrl/clientes/create", 'GET', null, $adminCookieFile, [], true);
    if ($clientesCreateResponse['http_code'] === 200) {
        ...
        if ($csrfToken) {
            ...
            if ($createResponse['http_code'] === 302) {
                ...
                $clientesListResponse = makeCurlRequest("$baseUrl/clientes", 'GET', null, $adminCookieFile, [], true);
    ...
    $vehiculosResponse = makeCurlRequest("$baseUrl/vehiculos", 'GET', null, $adminCookieFile, [], true);
    if ($vehiculosResponse['http_code'] === 200) {
        ...
        if (preg_match('/\/vehiculos\/(\d+)\/edit/', $vehiculosResponse['response'], $matches)) {
            ...
            $editResponse = makeCurlRequest("$baseUrl/vehiculos/$vehiculoId/edit", 'GET', null, $adminCookieFile, [], true);
            if ($editResponse['http_code'] === 200) {
                $csrfToken = extractCsrfToken($editResponse['response']);
                if ($csrfToken) {
    ...
    $empleadosResponse = makeCurlRequest("$baseUrl/empleados", 'GET', null, $adminCookieFile, [], true);
    if ($empleadosResponse['http_code'] === 200) {
        ...
        if (preg_match('/\/empleados\/(\d+)\/edit/', $empleadosResponse['response'], $matches)) {
            ...
            $editResponse = makeCurlRequest("$baseUrl/empleados/$empleadoId/edit", 'GET', null, $adminCookieFile, [], true);
            if ($editResponse['http_code'] === 200) {
                $csrfToken = extractCsrfToken($editResponse['response']);
                if ($csrfToken) {
    ...
    $serviciosResponse = makeCurlRequest("$baseUrl/servicios", 'GET', null, $adminCookieFile, [], true);
    if ($serviciosResponse['http_code'] === 200) {
        ...
        if (preg_match('/\/servicios\/(\d+)\/edit/', $serviciosResponse['response'], $matches)) {
            ...
            $editResponse = makeCurlRequest("$baseUrl/servicios/$servicioId/edit", 'GET', null, $adminCookieFile, [], true);
            if ($editResponse['http_code'] === 200) {
                $csrfToken = extractCsrfToken($editResponse['response']);
                if ($csrfToken) {
    ...
    $testResults['crud_operations']['admin_login'] = 'FAIL';
}
...
$vehiculosCreateResponse = makeCurlRequest("$baseUrl/vehiculos/create", 'GET', null, $adminCookieFile, [], true);
...
$empleadosCreateResponse = makeCurlRequest("$baseUrl/empleados/create", 'GET', null, $adminCookieFile, [], true);
...
$serviciosCreateResponse = makeCurlRequest("$baseUrl/servicios/create", 'GET', null, $adminCookieFile, [], true);
...
foreach (['mecanico', 'recepcion'] as $role) {
    ...
    $roleCookieFile = "/tmp/{$role}_permission_cookies_fixed.txt";
    ...
    if (loginUser($testUsers[$role]['email'], $testUsers[$role]['password'], $roleCookieFile)) {
        ...
        $testResults['permission_system']["{$role}_login"] = 'PASS';
        
        // Test access to empleados (should be denied for non-admin)
        $empleadosResponse = makeCurlRequest("$baseUrl/empleados", 'GET', null, $roleCookieFile, [], true);
        
        if ($empleadosResponse['http_code'] === 403 || strpos($empleadosResponse['response'], 'Unauthorized') !== false || strpos($empleadosResponse['response'], '403') !== false) {
            echo "✅ {$role} correctly denied access to empleados\n";
            $testResults['permission_system']["{$role}_empleados_access"] = 'PASS';
        } else {
            echo "❌ {$role} should not have access to empleados but does (HTTP {$empleadosResponse['http_code']})\n";
            $testResults['permission_system']["{$role}_empleados_access"] = 'FAIL';
        }
        
        // Test access to clientes (should be allowed)
        $clientesResponse = makeCurlRequest("$baseUrl/clientes", 'GET', null, $roleCookieFile, [], true);
        
        if ($clientesResponse['http_code'] === 200) {
            echo "✅ {$role} correctly has access to clientes\n";
            $testResults['permission_system']["{$role}_clientes_access"] = 'PASS';
        } else {
            echo "❌ {$role} should have access to clientes but doesn't (HTTP {$clientesResponse['http_code']})\n";
...
if ($clientesCreateResponse['http_code'] === 200) {
    ...
    if ($csrfToken) {
        ...
        if ($formResponse['http_code'] === 302) {
            ...
            echo "❌ Traditional form submission failed (HTTP {$formResponse['http_code']})\n";
...
if ($csrfToken) {
    ...
    $ajaxResponse = makeCurlRequest("$baseUrl/clientes", 'POST', $ajaxData, $adminCookieFile, $ajaxHeaders, false);
    ...
    if ($ajaxResponse['http_code'] === 200 || $ajaxResponse['http_code'] === 302) {
        ...
        echo "❌ AJAX form submission failed (HTTP {$ajaxResponse['http_code']})\n";
...
$unauthCookieFile = '/tmp/unauth_cookies_fixed.txt';
...
foreach ($protectedRoutes as $route) {
    $response = makeCurlRequest("$baseUrl$route", 'GET', null, $unauthCookieFile, [], false);
    
    if ($response['http_code'] === 302) {
        echo "✅ $route correctly redirects unauthenticated users\n";
        $testResults['route_protection'][$route] = 'PASS';
    } else {
        echo "❌ $route should redirect unauthenticated users but returns {$response['http_code']}\n";
...
$notFoundResponse = makeCurlRequest("$baseUrl/clientes/99999", 'GET', null, $adminCookieFile, [], true);
...
if ($testResults['summary']['success_rate'] >= 80) {
    echo "✅ EXCELLENT: System is working very well with {$testResults['summary']['success_rate']}% success rate\n";
} elseif ($testResults['summary']['success_rate'] >= 60) {
    echo "⚠️ GOOD: System is mostly working with {$testResults['summary']['success_rate']}% success rate\n";
} else {
    echo "❌ NEEDS ATTENTION: System has issues with only {$testResults['summary']['success_rate']}% success rate\n";
}
...
file_put_contents('/mnt/c/Users/lukka/taller-sistema/comprehensive_final_verification_report_fixed.json', json_encode($testResults, JSON_PRETTY_PRINT));

echo "\nDetailed test report saved to: comprehensive_final_verification_report_fixed.json\n";
